...
from .ibmqasm3Lexer import ibmqasm3Lexer
from .ibmqasm3Parser import ibmqasm3Parser

__all__ = ["pretty_tree", "tokens_parser"]

def tokens_parser(*, program: str = None, file: str = None):
    ...
    with io.StringIO() as err, contextlib.redirect_stderr(err):
        lexer = ibmqasm3Lexer(input_stream)
        token_stream = antlr4.CommonTokenStream(lexer)
        parser = ibmqasm3Parser(token_stream)
        error = err.getvalue()
    if error:
        raise Qasm3ParserError(f"Parse tree build failed. Error:\n{error}")
    return token_stream, parser
